...
const TermsAndConditions = () => {
  ...
  useEffect(() => {
    const fetchPolicy = async () => {
      try {
        const res = await api.get('/policies/terms');
  ...
  return (
    <div className="container mx-auto px-4 py-4 md:py-8 max-w-4xl font-sans">
      <div className="text-center mb-6">
        <h1 className="text-xl md:text-2xl font-serif font-black text-brand-dark uppercase tracking-widest mb-2">Terms & Conditions</h1>
      ...
      <div className="bg-white/50 backdrop-blur-sm p-6 md:p-8 rounded-2xl border border-brand-pink/10 shadow-xl shadow-brand-pink/[0.02] space-y-6">
        <div 
          ...
          dangerouslySetInnerHTML={{ __html: policy?.content || 'Terms & Conditions coming soon...' }}
        />
